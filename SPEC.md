...
- [ ] **report-02**: `src/health/report/daily.py` — one-day card: yesterday's activity, sleep, HRV, body battery, vs 7d mean.
- [ ] **report-03**: `src/health/report/weekly.py` — KPI dashboard (load, physiology, activity), week-over-week trend, top anomalies.
...
- [ ] **plan-01**: `src/health/plan/schema.py` — Pydantic model for `plan.yaml`.
- [ ] **plan-02**: `src/health/plan/loader.py` — load + validate plan.
- [ ] **plan-03**: `src/health/plan/adherence.py` — compare a week's activities/sleep/load against `weekly_targets`. Output a per-target score (0–100), an overall weighted score, and a list of misses with deltas.
